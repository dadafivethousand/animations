...
export default function CaptainAmericaSchedule({ day, animationDelay = 1000, animationInterval = 300 }) {
  ...
  return (
    <div className="cap-wrapper">
      <div className="cap-shield-bg" />
      <h2 className="cap-day">{day.toUpperCase()}</h2>
      <div className="cap-grid">
        {(schedule[day] || []).map((cls, idx) =>
          visibleArray.includes(idx) ? (
            <div className="cap-card" key={idx}>
              <span className="cap-class">{cls.name}</span>
              <span className="cap-time">{formatTime(cls.start)}</span>
            </div>
          ) : null
        )}
      </div>
